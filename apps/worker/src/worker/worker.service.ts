    ...
    try {
      const claimedJobs = await this.prisma.$transaction(async (tx) => {
        const queues = await tx.$queryRaw<any[]>`
          SELECT concurrency_limit, current_running, rate_limit_per_second FROM queues WHERE id = ${queueId}::uuid FOR UPDATE;
        `;
        ...
        let capacity = Math.min(workerCapacity, queueCapacity);
  ...
  private async executeJob(job: any) {
    ...
    try {
      ...
      await this.prisma.$transaction(async (tx) => {
        ...
        // Batch progress update on completion
        if (job.batch_id) {
  ...
  private async handleJobFailure(job: any, executionId: bigint, startTime: Date, error: Error) {
    ...
    try {
      await this.prisma.$transaction(async (tx) => {
        ...
        // Batch progress update on failure
